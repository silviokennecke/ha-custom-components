import{LitElement,html,css}from"https://unpkg.com/lit-element@2.0.1/lit-element.js?module";class PublicTransportConnectionCard extends LitElement{static getStubConfig(){return{title:"Next train",departure_station:"Home",arrival_station:"Work",entity:"sensor.my_station_to_another_station",attributes:{description:"products",departure_time:"departure",departure_delay:"delay",arrival_time:"arrival",arrival_delay:"delay_arrival",next_departure_time:"next",next_arrival_time:"next_on"},theme:"deutsche-bahn"}}static get properties(){return{hass:{},config:{}}}render(){const title=this.config.title;const entity=this.config.entity;const theme=this.config.theme;const stateObj=this.hass.states[entity];if(!stateObj){return html`
        <ha-card class="ptc-theme-${theme}">
          ${title?html`<h1>${title}</h1>`:""}
          <div class="not-found">Entity ${entity} not found.</div>
        </ha-card>
      `}const icon=this.config.icon||stateObj.attributes.icon||"mdi:train";const connections={current:{description:stateObj.attributes[this.config.attributes.description].join(", ")||"",departure:{time:stateObj.attributes[this.config.attributes.departure_time],delay:stateObj.attributes[this.config.attributes.departure_delay]||"",station:stateObj.attributes[this.config.attributes.departure_station]||this.config.departure_station||""},arrival:{time:stateObj.attributes[this.config.attributes.arrival_time],delay:stateObj.attributes[this.config.attributes.arrival_delay]||"",station:stateObj.attributes[this.config.attributes.arrival_station]||this.config.arrival_station||""}}};if(this.config.attributes.next_departure_time&&this.config.attributes.next_arrival_time){connections.next={description:stateObj.attributes[this.config.attributes.next_description]||"",departure:{time:stateObj.attributes[this.config.attributes.next_departure_time],delay:stateObj.attributes[this.config.attributes.next_departure_delay]||"",station:stateObj.attributes[this.config.attributes.next_departure_station]||this.config.departure_station||""},arrival:{time:stateObj.attributes[this.config.attributes.next_arrival_time],delay:stateObj.attributes[this.config.attributes.next_arrival_delay]||"",station:stateObj.attributes[this.config.attributes.next_arrival_station]||this.config.arrival_station||""}}}return html`
      <ha-card class="ptc-theme-${theme}">
        ${title?html`<h1>${title}</h1>`:""}
