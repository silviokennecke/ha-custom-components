import{LitElement,html,css}from"https://unpkg.com/lit-element@2.0.1/lit-element.js?module";class PublicTransportConnectionCard extends LitElement{static getStubConfig(){return{title:"Next train",departure_station:"Home",arrival_station:"Work",entity:"sensor.my_station_to_another_station",connection_attributes:{connections:"departures",description:"products",departure_time:"departure",departure_delay:"delay",arrival_time:"arrival",arrival_delay:"delay_arrival"},displayed_connections:3,theme:"deutsche-bahn"}}static get properties(){return{hass:{},config:{}}}render(){const title=this.config.title;const entity=this.config.entity;const theme=this.config.theme;const stateObj=this.hass.states[entity];if(!stateObj){return html`
        <ha-card class="ptc-theme-${theme}">
          ${title?html`<h1>${title}</h1>`:""}
          <div class="not-found">Entity ${entity} not found.</div>
        </ha-card>
      `}const icon=this.config.icon||stateObj.attributes.icon||"mdi:train";const connections={current:{},next:[]};const useAttributes=!!this.config.attributes;const useConnections=!!this.config.connection_attributes;if(useAttributes){const description=stateObj.attributes[this.config.attributes.description]||"";connections.current={description:Array.isArray(description)?description.join(", "):description,departure:{time:stateObj.attributes[this.config.attributes.departure_time],delay:stateObj.attributes[this.config.attributes.departure_delay]||"",station:stateObj.attributes[this.config.attributes.departure_station]||this.config.departure_station||""},arrival:{time:stateObj.attributes[this.config.attributes.arrival_time],delay:stateObj.attributes[this.config.attributes.arrival_delay]||"",station:stateObj.attributes[this.config.attributes.arrival_station]||this.config.arrival_station||""}};if(this.config.attributes.next_departure_time&&this.config.attributes.next_arrival_time){const nextDescription=stateObj.attributes[this.config.attributes.next_description]||"";connections.next=[{description:Array.isArray(nextDescription)?nextDescription.join(", "):nextDescription,departure:{time:stateObj.attributes[this.config.attributes.next_departure_time],delay:stateObj.attributes[this.config.attributes.next_departure_delay]||"",station:stateObj.attributes[this.config.attributes.next_departure_station]||this.config.departure_station||""},arrival:{time:stateObj.attributes[this.config.attributes.next_arrival_time],delay:stateObj.attributes[this.config.attributes.next_arrival_delay]||"",station:stateObj.attributes[this.config.attributes.next_arrival_station]||this.config.arrival_station||""}}]}}else if(useConnections){const nextConnections=stateObj.attributes[this.config.connection_attributes.connections]||[];for(let i=0;i<this.config.displayed_connections&&i<nextConnections.length;i++){const nextConnection=nextConnections[i];if(nextConnection===undefined){continue}const nextDescription=nextConnection[this.config.connection_attributes.description]||"";const displayedConnection={description:Array.isArray(nextDescription)?nextDescription.join(", "):nextDescription,departure:{time:nextConnection[this.config.connection_attributes.departure_time],delay:nextConnection[this.config.connection_attributes.departure_delay]||"",station:nextConnection[this.config.connection_attributes.departure_station]||this.config.departure_station||""},arrival:{time:nextConnection[this.config.connection_attributes.arrival_time],delay:nextConnection[this.config.connection_attributes.arrival_delay]||"",station:nextConnection[this.config.connection_attributes.arrival_station]||this.config.arrival_station||""}};if(i===0){connections.current=displayedConnection}else{connections.next.push(displayedConnection)}}}return html`
      <ha-card class="ptc-theme-${theme}">
        ${title?html`<h1>${title}</h1>`:""}
    ...
    `}setConfig(config){if(!config.entity){throw new Error("You need to define an entity")}const useAttributes=!!config.attributes;const useConnectionsList=!!config.connection_attributes;if(!useAttributes&&!useConnectionsList){throw new Error("You need to define attributes or connection_attributes")}else if(useAttributes&&useConnectionsList){throw new Error("Yout cann only use attributes or connection_attributes. You cannot use both")}if(useAttributes){if(!!config.displayed_connections){throw new Error("You cannot define displayed_connections, when using attributes")}if(!config.attributes.departure_time){throw new Error("You need to define the departure attribute")}if(!config.attributes.arrival_time){throw new Error("You need to define the arrival attribute")}if(config.attributes.next_departure_time&&!config.attributes.next_arrival_time){throw new Error("If you define the next_departure attribute, you need to also define the next_arrival attribute")}}if(useConnectionsList){if(!config.displayed_connections||config.displayed_connections<1){throw new Error("displayed_connections must be set to 1 or higher")}if(!config.connection_attributes.connections){throw new Error("You must define the connections attribute")}if(!config.connection_attributes.departure_time){throw new Error("You must define the departure_time attribute for connection entries")}if(!config.connection_attributes.arrival_time){throw new Error("You must define the arrival_time attribute for connection entries")}}this.config={tap_action:{action:"more-info"},...config}}getCardSize(){return 2}_handleAction(action){const event=new Event("hass-action",{bubbles:true,composed:true});event.detail={config:this.config,action:action};this.dispatchEvent(event)}static get styles(){return css`
      :host {
